...
public readonly record struct ColorInfo(byte A, byte R, byte G, byte B)
{
    ...
    public static ColorInfo FromHexColorString(string colorString)
    {
        ...
        {
            if (!byte.TryParse(colorString[1..2], NumberStyles.HexNumber, null, out var r) ||
                    !byte.TryParse(colorString[3..4], NumberStyles.HexNumber, null, out var g) ||
                    !byte.TryParse(colorString[5..6], NumberStyles.HexNumber, null, out var b))
            {
                return default;
            }
        ...
        {
            if (!byte.TryParse(colorString[1..2], NumberStyles.HexNumber, null, out var a) ||
                    !byte.TryParse(colorString[3..4], NumberStyles.HexNumber, null, out var r) ||
                    !byte.TryParse(colorString[5..6], NumberStyles.HexNumber, null, out var g) ||
                    !byte.TryParse(colorString[7..8], NumberStyles.HexNumber, null, out var b))
            {
                return default;
            }
    ...
    {
        var r = Convert.ToByte((value + R)).Clamp(byte.MinValue, byte.MaxValue);
        var g = Convert.ToByte((value + G)).Clamp(byte.MinValue, byte.MaxValue);
        var b = Convert.ToByte((value + B)).Clamp(byte.MinValue, byte.MaxValue);
